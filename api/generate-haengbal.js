...
5. 학생의 학교생활 태도, 책임감, 자기관리, 자기주도성, 친구 관계, 소통과 협력 등을 중심으로 작성하세요.
6. "행복해요", "좋아요", "없음"처럼 구체적인 근거가 부족한 표현은 과도하게 해석하지 마세요.
7. 학생이 어려움을 해결하려고 노력했거나 행동의 변화를 직접 작성한 경우에는 이를 반영하세요.
8. 과장하거나 지나치게 긍정적으로 표현하지 마세요.
9. "~함", "~보임", "~나타냄", "~노력함" 등의 학교생활기록부 문체를 사용하세요.
10. 교사가 실제로 관찰하고 기록한 것처럼 자연스럽게 작성하세요.
11. 결과만 출력하고 제목이나 설명은 출력하지 마세요.
12. 생기부 문장에는 가운데점(·)을 사용하지 마세요. 가운데점 대신 쉼표(,)를 사용하세요.
13. 지나치게 긴 문장 하나로 작성하지 말고 2~3개의 자연스러운 문장으로 작성하세요.
...
`;
    ...
    function cutTo1000Bytes(text) {
      ...
      for (const char of text) {
        ...
        if (
          new TextEncoder().encode(test).length > 1000
        ) {
          break;
        }
